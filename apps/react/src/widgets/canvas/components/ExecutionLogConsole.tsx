    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={`absolute bottom-4 ${isMinimized ? 'left-14 ' : 'left-4'} z-40 bg-[var(--header-bg)] border border-[var(--border)] rounded-[var(--radius)] shadow-xl flex flex-col overflow-hidden backdrop-blur-md select-none`}
        >
          {isMinimized ? (
            <button
