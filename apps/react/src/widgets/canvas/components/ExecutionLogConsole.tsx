      ...
      {isVisible && (
        ...
        >
          {isMinimized ? (
            ...
            <>
              <div
                className="flex shrink-0 items-center justify-between border-b border-border bg-accent px-4 py-2 text-foreground">
                <div
                ...
                >
                  ...
                  {status === 'running' && (
                    <span
                      className="inline-flex animate-pulse items-center rounded-full bg-(--node-output)/10 px-1.5 py-0.5 text-[10px] font-medium text-(--node-output)">
                      running
                    </span>
                  )}
                  {status === 'failed' && (
                    <span
                      className="inline-flex items-center rounded-full bg-(--node-math)/10 px-1.5 py-0.5 text-[10px] font-medium text-(--node-math)">
                      failed
                    </span>
                  )}
                  {status === 'success' && (
                    <span
                      className="inline-flex items-center rounded-full bg-(--node-output)/10 px-1.5 py-0.5 text-[10px] font-medium text-(--node-output)">
                      success
                    </span>
                  )}
                </div>
              ...
              <div
                className="flex shrink-0 items-center gap-4 border-b border-border bg-(--background)/30 px-4 py-1 text-[10px] font-medium text-muted-foreground">
              ...
              <div
                className="flex-1 scrollbar-thin space-y-1.5 overflow-y-auto bg-(--background)/50 p-3 font-mono text-[11px]">
                {logs.map((log) => (
                  <div key={log.id} className="flex items-start gap-2 leading-relaxed">
                    <span className="text-foreground opacity-40">[{log.timestamp}]</span>
                    <span
                      className={
                        log.type === 'error'
                          ? 'text-(--node-math)'
